...
There are two types of authentication types that can be used to anchor through Niomon:

* With a Password: This password can be found on the console. It has to be encoded in base64.
* With a JWT: An Ubirch JWT with scopes  upp:anchor.
...
CREDENTIAL=base64 enconded password or JWT(upp:anchor)
DATA='/PATH_TO_UPP.der'

curl -X POST 'https://niomon.prod.ubirch.com/' \
--header "X-Ubirch-Auth-Type: $AUTH_TYPE" \
--header "X-Ubirch-Hardware-Id: $HARDWARE_ID" \
--header "X-Ubirch-Credential: $CREDENTIAL" \
